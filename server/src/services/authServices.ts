...
const authError = { code: 401, message: 'Неверное имя или пароль.' };
...
const refreshTokenRequest = async (refreshToken: string) => {
  try {
    ...
    throw { code: 401, message: 'Недействительный или просроченный токен обновления.' };
  }
};
...
const changePasswordRequest = async (
  ...
  if (!registeredUser) {
    throw { code: 404, message: 'Пользователь не найден.' };
  }

  const isMatch = await bcrypt.compare(oldPassword, registeredUser.password);
  if (!isMatch) {
    throw { code: 400, message: 'Неверный старый пароль.' };
  }
