...
const createRatingRequest = async (
  ...
  try {
    ...
    if (!book) throw { code: 404, message: 'Документ не найден' };
    if (!user) throw { code: 404, message: 'Невозможно найти пользователя' };
...
const destroyRatingRequest = async (req: Request, ratingId: string) => {
  ...
  if (!rating) {
    throw { code: 404, message: `Ошибка: рейтинг с таким идентификатором отсутствует ${ratingId}` };
  }

  if (rating.userId !== (req as any).user.id) {
    throw { code: 403, message: 'Вы можете удалять только свои собственные оценки' };
  }
...
const updateRatingRequest = async (
  ...
  if (!rating) {
    throw { code: 404, message: `Ошибка: рейтинг с таким идентификатором отсутствует${id}` };
  }

  if (rating.userId !== (req as any).user.id) {
    throw { code: 403, message: 'Вы можете обновлять только свои собственные рейтинги.' };
  }
