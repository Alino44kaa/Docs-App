...
describe('fetchDataWrapper', () => {
  ...
  it('should call toast.error when callback fails', async () => {
    const mockError = new Error('Что-то пошло не так');
  ...
  it('should handle non-Error objects in the catch block', async () => {
    const mockCallback = jest.fn().mockRejectedValueOnce('Сообщение об ошибке');
    ...
    expect(setIsLoading).toHaveBeenCalledWith(true);
    expect(setIsLoading).toHaveBeenCalledWith(false);
    expect(toast.error).toHaveBeenCalledWith('Сообщение об ошибке');
  });
});
